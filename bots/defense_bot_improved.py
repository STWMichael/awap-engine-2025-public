...
class BotPlayer(Player):
    ...
    def play_turn(self, rc: RobotController):
        ...
        # STEP 3: Classify our units (only Knights are classified for defense/offense).
        ...
        # BONUS: Let healers follow offensive knights.
    ...
    def clear_spawn_tile(self, rc: RobotController, main_castle):
        """Move any unit (Knight, Catapult, or Land Healer) off the castle tile (spawn point)."""
        team = rc.get_ally_team()
        for uid in rc.get_unit_ids(team):
            unit = rc.get_unit_from_id(uid)
            if not unit:
                continue
            if unit.type in (UnitType.KNIGHT, UnitType.CATAPULT, UnitType.LAND_HEALER_1) and unit.x == main_castle.x and unit.y == main_castle.y:
    ...
    def spawn_units(self, rc: RobotController, main_castle_id: int, turn: int, enemy_has_catapult: bool):
        team = rc.get_ally_team()
        balance = rc.get_balance(team)
        knight_ids = [uid for uid in rc.get_unit_ids(team)
                      if rc.get_unit_from_id(uid) and rc.get_unit_from_id(uid).type == UnitType.KNIGHT]
        ...
        # Spawn knights until we reach our defensive target.
        defense_knight_target = 10
        if num_knights < defense_knight_target:
            if rc.can_spawn_unit(UnitType.KNIGHT, main_castle_id):
                rc.spawn_unit(UnitType.KNIGHT, main_castle_id)
                return
        ...
        # Fallback: spawn additional knights.
        if rc.can_spawn_unit(UnitType.KNIGHT, main_castle_id):
            rc.spawn_unit(UnitType.KNIGHT, main_castle_id)
    ...
    def release_defensive_units(self, rc: RobotController, main_castle):
        ...
        if not threat_detected:
            for uid in rc.get_unit_ids(team):
                ...
                if not unit:
                    continue
                if unit.type in (UnitType.KNIGHT, UnitType.CATAPULT) and rc.get_chebyshev_distance(unit.x, unit.y, main_castle.x, main_castle.y) < 4:
                    dirs = rc.unit_possible_move_directions(uid)
                    if dirs:
    ...
    def release_offensive_path(self, rc: RobotController, main_castle, enemy_castle):
        ...
        for uid in rc.get_unit_ids(team):
            ...
            if not unit:
                continue
            if unit.type in (UnitType.KNIGHT, UnitType.CATAPULT) and (unit.x, unit.y) in corridor:
    ...
    def clear_defensive_corridor_blockers(self, rc: RobotController, main_castle, enemy_castle):
        ...
        for uid in rc.get_unit_ids(team):
            ...
            if not unit:
                continue
            if unit.type == UnitType.KNIGHT and (unit.x, unit.y) in corridor:
    ...
    #                     enemy_unit_to_attack = enemy_unit
    #                     break
    #             if enemy_unit_to_attack:
    #                 rc.unit_attack_unit(uid, enemy_unit_to_attack.id)
    #             else:
    #                 dirs = rc.unit_possible_move_directions(uid)
    #                 dirs.sort(key=lambda d: rc.get_chebyshev_distance(*rc.new_location(unit.x, unit.y, d),
